(function installV2RecordsUi() {
  ...
  renderMealReportSection = function renderV2MealReportSection() {
    ...
    return `<section class="records-section">
      ...
      <div class="records-table-wrap"><table class="records-table"><thead><tr><th>日期</th><th>單位</th><th>員工</th><th>品項</th><th>數量</th><th>單價</th><th>小計</th><th>備註</th><th>首次下訂</th><th>最後修改</th><th>警告</th></tr></thead><tbody>${(report.details || []).map((row) => `<tr><td>${escapeHtml(row.date || "")}</td><td>${escapeHtml(row.departmentName || "")}</td><td>${escapeHtml(row.employeeName || "")}<br><span>${escapeHtml(row.employeeCode || "")}</span></td><td>${escapeHtml(row.productName || "")}</td><td>${Number(row.quantity || 0)}</td><td>$${Number(row.unitPrice || 0).toFixed(0)}</td><td>$${Number(row.amount || 0).toFixed(0)}</td><td>${escapeHtml(row.note || "")}</td><td>${formatRecordDateTime(row.submittedAt)}</td><td>${formatRecordDateTime(row.updatedAt)}</td><td>${row.clockDeletedWarning ? "上班打卡已刪除" : ""}</td></tr>`).join("") || '<tr><td colspan="11">沒有訂餐資料</td></tr>'}</tbody></table></div>
