...
1. 純 UI 文字、按鈕標籤、單一欄位顯示或局部互動，原則上只修改一個對應的 canonical Renderer／模組與必要測試；`src/renderer/app.js`、`docs/app.js`、`docs/` 等建置產物不算第二份正式來源，也不得人工同步修改。
...
6. 回歸測試應直接鎖住 canonical 行為與正式輸出，不鎖住補丁、DOM 後處理或舊相容流程。
