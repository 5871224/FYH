...
小型修改應優先維持「改一個正式來源，就得到一個明確結果」，避免讓簡單需求因重複來源、runtime patch 或產生檔同步而擴大成跨檔追查。

1. 純 UI 文字、按鈕標籤、單一欄位顯示或局部互動，原則上只修改一個對應的 canonical Renderer／模組；只有實際行為風險需要覆蓋時才新增或更新測試。`src/renderer/app.js`、`docs/app.js`、`docs/` 等建置產物不算第二份正式來源，也不得人工同步修改。
2. 同一業務常數或顯示值若必須在多個 runtime 模組手動重複修改，應優先抽成正式常數／helper，或回到真正的資料來源統一；不得靠載入後字串取代把錯誤值「修成正確值」。
3. i18n 只負責語言翻譯與錯誤訊息正規化，不得改寫業務值、ID、日期、數值、密碼預設值或其他資料內容。像 `000000` 這類正式值必須由 Renderer／業務來源直接產生，不能由語系層把 `0000` 補成六碼。
4. JavaScript／CSS 正式來源修改後，一律由既有 build／publish 流程產生 bundle 與 `docs/`；不得為了省事直接改產生檔，也不得建立一次性 workflow 幫忙改寫。
5. 若一個明確的小需求經常需要同時修改超過兩個正式來源模組、追查 DOM 後處理、wrapper、override 或相容層，視為架構邊界過度耦合的訊號；應先消除重複責任或收斂 source of truth，而不是再加下一層補丁。
6. 回歸測試優先驗證使用者可觀察行為、API 結果、資料安全與資料一致性；不得只為鎖住函式名稱、檔案位置、CSS selector、字串實作、舊名稱不存在或某段原始碼長相而新增測試。
...
8. 是否需要測試以風險與可觀察行為為準；禁止用大量原始碼掃描測試來替代程式碼審查或架構整理。
...
測試優先覆蓋權限隔離、資料一致性、登入與帳號安全、打卡、訂餐、排班、匯出結果及重要使用者流程。純文字、按鈕標籤、函式命名、檔案位置、selector 或實作方式本身通常不建立測試；這類事項由程式碼審查與既有建置檢查處理。
