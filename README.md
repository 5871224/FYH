...
本 README 已整合「福圓號排班系統擴充規格書 V2.0」的目前需求。系統以手機優先，正式時間判定一律使用伺服器端 `Asia/Taipei`，不得用使用者本機時間作為登入逾時、到離職有效期、打卡、加班申請期限、訂餐日期與截止時間的正式判定。

角色分為：

- `employee`：查看自己的班表、打卡、今日訂餐、個人記錄、提出與期限內重提自己的加班申請。
- `manager`：具備員工權限，可管理班表、員工與主管的非權限資料、一般單位、訂餐商品與截止時間，並查看及匯出訂餐統計。
- `admin`：具備全部功能，可管理角色、管理員、打卡設定、打卡補登/修改/刪除、打卡稽核資料、加班審核與批次操作。

V2 頁面與權限重點：

- 首頁只顯示主要入口；管理功能依角色顯示，不讓員工看到主管/管理員工具。
- 打卡使用伺服器端有效任職檢查、單位打卡設定、GPS/IP/距離資料與異常標記。管理員可補登或修改上下班時間，修改歷程寫入 `attendance_action_logs`。
- 加班以 `attendance_overtime_requests` 為正式申請來源，審核歷程寫入 `overtime_review_logs`。員工可在期限內刪除待審或退回申請後重新申請；管理員可審核、退回、調整核准時數與代為申請。
- 訂餐以 `meal_orders` 為正式訂單來源，不另設訂單主檔。商品與價格在 `meal_products`，截止時間在 `meal_settings`。主管與管理員可設定商品、截止時間、查看訂餐統計與匯出 Excel。
- 記錄頁權限：員工只有個人記錄；主管另有訂餐統計；管理員另有訂餐統計、加班審核、打卡管理。

驗收重點：

- 員工不能看到或呼叫主管/管理員資料與操作。
- 主管不能建立或修改管理員、不能使用打卡管理與加班審核。
- 管理員操作應留下必要歷程，且不因空白原因阻擋打卡補登或修改。
