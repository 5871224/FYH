...
assert(memberAuthAdmin.includes("function assertLastAdminProtected"), "member auth sync should protect the last admin");
...
assert(webApi.includes("function assertSessionActive"), "web api should enforce idle timeout on every authenticated request");
...
assert(attendanceClock.includes('rpc("save_attendance_clock"') && schema.includes("create or replace function public.save_attendance_clock"), "attendance clock should use the atomic RPC");
...
assert(attendanceOvertime.includes("earlyHours > eligibility.earlyHours") && attendanceOvertime.includes("lateHours > eligibility.lateHours"), "attendance overtime should reject employee hours above system calculation");
...
assert(!renderer.includes('data-set-department-view="member"'), "department settings should keep only the department view");
