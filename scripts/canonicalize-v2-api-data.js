...
function replaceFunctionBlock(source, startName, endName, replacement) {
  ...
  if (start < 0 || end <= start) throw new Error(`找不到 API 區段：${startName} -> ${endName}`);
...
webApi = replaceFunctionBlock(webApi, "getTodayAttendanceOvertime", "getMealOrderStatus", overtimeFunctions);
...
if (loadStateStart < 0 || loadStateEnd <= loadStateStart) throw new Error("找不到 loadState 區段");
...
const exportReplacement = `    getEmployeeOvertimeDates,
    ...
    getMemberOrder,
    saveMemberOrder,`;
if (!webApi.includes(exportMarker)) throw new Error("找不到 V2 API 輸出位置");
webApi = webApi.replace(exportMarker, exportReplacement);
...
if (!build.includes('"v2-api.js"')) throw new Error("建置清單找不到 v2-api.js");
build = build.replace('"v2-api.js"', '"v2-tablet-session.js"');
fs.writeFileSync(buildPath, build);

for (const checkPath of [finalCheckPath, alignmentPath]) {
  let source = fs.readFileSync(checkPath, "utf8");
  source = source.replaceAll("src/renderer/v2-api.js", "src/renderer/v2-tablet-session.js");
...
test("員工加班 API 應保留日期、狀態、送出與刪除操作", async () => {
  const source = readWebApi();
  const start = source.indexOf("async function getEmployeeOvertimeDates");
  const end = source.indexOf("async function getMealOrderStatus", start);
  const calls = [];
  const context = {
