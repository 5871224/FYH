...
const testPath = path.join(root, "tests", "renderer-phase6-settings.test.js");

const blocks = [
  { file: "renderer-settings-catalog.js", start: "openListSettings", end: "openDepartmentSettings" },
  { file: "renderer-settings-department.js", start: "openDepartmentSettings", end: "getOrderedIdsFromDom" },
  { file: "renderer-settings-ordering.js", start: "getOrderedIdsFromDom", end: "buildSelectOptions" },
  { file: "renderer-settings-member.js", start: "buildSelectOptions", end: "syncScheduleCatalogs" }
];
...
for (const range of ranges) fs.writeFileSync(path.join(rendererDir, range.file), range.content);
...
console.log(`renderer phase 6 prepared: ${ranges.length} settings blocks moved; renderer lines: ${renderer.split(/\r?\n/).length}`);
