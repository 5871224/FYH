...
test("簽到審核匯出使用正式 exportRows 十二欄契約並納入休例日排班", () => {
  ...
  assert.match(attendance, /schedule_entries/);
  assert.match(attendance, /set_shift/);
  assert.match(attendance, /set_leave/);
  assert.match(attendance, /leave\?\.code==='0036'/);
  assert.match(attendance, /leave\?\.code==='0047'/);
  assert.match(attendance, /leave\?\.name==='例假'/);
  assert.match(attendance, /leave\?\.name==='休息日'/);
  assert.match(attendance, /restDayScheduled:rest/);
  assert.match(attendance, /scheduledShiftStartTime:rest\?row\.shift\.start_time/);
  assert.match(attendance, /scheduledShiftEndTime:rest\?row\.shift\.end_time/);
