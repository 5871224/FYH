...
test("班表歷程控制應由正式互動模組提供而非覆蓋函式", () => {
  ...
  assert.equal(rendererCore.includes("await (redoRequested ? redoSchedule() : undoSchedule())"), true);
  assert.equal(rendererCore.includes("bindScheduleHistoryControls();"), true);
});
