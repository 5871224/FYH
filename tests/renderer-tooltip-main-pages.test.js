...
const recordsViews = read("src/renderer/renderer-records-views.js");
...
test("主頁渲染應保留三個入口、簽到簿名稱與訂餐管理分頁", () => {
  assert.equal(pages.includes('<div class="clock-page-header">'), true);
  assert.equal(pages.includes("home-hero"), false);
  ["data-home-action=\"schedule\"", "data-home-action=\"meal\"", "data-home-action=\"records\""].forEach((marker) => assert.equal(pages.includes(marker), true));
  assert.equal(pages.includes("data-home-action=\"clock\""), false);
  assert.equal(pages.includes('<span class="home-action-title">簽到簿</span>'), true);
  assert.equal(recordsViews.includes('aria-label="簽到簿分頁"'), true);
  assert.equal(pages.includes("data-meal-tab=\"stats\""), true);
  assert.equal(pages.includes("data-meal-tab=\"settings\""), true);
  assert.equal(pages.includes("renderTodayOvertimePanel()"), false);
});
...
test("第十二階段應移出提示框與主頁渲染並維持模組順序", () => {
  ...
  ["shouldPromptLeaveDetail", "showScheduleTooltip", "renderHomeDashboard", "renderMealPage"].forEach((name) => {
    assert.equal(renderer.includes("function " + name), false, "renderer.js 仍保留 " + name);
  });
  assert.equal(renderer.includes("function renderClockPage"), false);
