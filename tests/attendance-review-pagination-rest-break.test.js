...
test("簽到審核換頁走 FYH API 並防止過期請求覆蓋", () => {
  ...
  assert.match(webApi, /async function request\(pathname, options = \{\}\)[\s\S]*?await fetch/);
  assert.match(webApi, /getAttendanceReviewList[\s\S]*?\/api\/v1\/attendance\/review\/list/);
  assert.doesNotMatch(webApi, /\brequestFunction\s*\(/);
...
test("休息日或例假簽到審核匯出在加班開始四小時後帶一小時休息", () => {
  ...
  assert.match(webApi, /function addMinutes\(value,delta\)/);
  const helperStart = webApi.indexOf("function addMinutes(value,delta)");
  const helperEnd = webApi.indexOf("\n  async function exportSapCsv", helperStart);
  const helperSource = webApi.slice(helperStart, helperEnd).trim();
  assert.ok(helperStart >= 0 && helperEnd > helperStart);
  const helper = Function(`${helperSource}; return addMinutes;`)();
  assert.equal(helper("11:30", 4 * 60), "15:30");
  assert.equal(helper("11:30", 5 * 60), "16:30");

  assert.match(webApi, /otStart=adjusted\.time\|\|start/);
  assert.match(webApi, /addMinutes\(otStart,240\)/);
  assert.match(webApi, /addMinutes\(otStart,300\)/);
  assert.match(webApi, /overtime_use_rest_1:true/);
  assert.match(webApi, /overtime_rest_1_start_time:addMinutes\(otStart,240\)/);
  assert.match(webApi, /overtime_rest_1_end_time:addMinutes\(otStart,300\)/);
